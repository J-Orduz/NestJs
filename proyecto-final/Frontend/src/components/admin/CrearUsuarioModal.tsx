...
interface CrearUsuarioModalProps {
  onClose: () => void;
  onUsuarioCreado: (usuarioId: string) => void;
}

const CrearUsuarioModal: React.FC<CrearUsuarioModalProps> = ({ onClose, onUsuarioCreado }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onUsuarioCreado(response.id); // Pasamos el ID automáticamente
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-md w-full">
        <div className="bg-gradient-to-r from-primary to-blue-600 rounded-t-2xl p-6 text-white">
          ...
          <p className="text-sm opacity-90">Ingresa los datos del nuevo administrador</p>
        </div>
