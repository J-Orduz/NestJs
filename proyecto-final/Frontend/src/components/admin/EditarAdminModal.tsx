// src/components/admin/EditarAdminModal.tsx
import React, { useState } from 'react';
import { adminService } from '../../services/api';

interface EditarAdminModalProps {
  admin: any;
  onClose: () => void;
  onAdminActualizado: () => void;
}

const EditarAdminModal: React.FC<EditarAdminModalProps> = ({ admin, onClose, onAdminActualizado }) => {
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  
  const [formData, setFormData] = useState({
    nombre_completo: admin.usuario?.nombre_completo || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.nombre_completo && formData.nombre_completo !== admin.usuario?.nombre_completo) {
      updateData.nombre_completo = formData.nombre_completo;
    }
    if (formData.correo && formData.correo !== admin.usuario?.correo) {
    ...
    try {
      await adminService.actualizarAdministrador(admin.id, updateData);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-md w-full">
        <div className="bg-gradient-to-r from-yellow-500 to-orange-500 rounded-t-2xl p-6 text-white">
          ...
          <p className="text-sm opacity-90">Modificar datos de {admin.usuario?.nombre_completo}</p>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex space-x-3">
            ...
            <button type="submit" disabled={submitting} className="flex-1 bg-yellow-500 text-white py-2 rounded-lg hover:bg-yellow-600 transition disabled:opacity-50">
...
export default EditarAdminModal;